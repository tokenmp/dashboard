...
import { getRedeemCodesApi, getCodeRedemptionsApi } from '@/api/redeem';
...
function RedeemCodes() {
  ...
  const { list, total, page, size, loading, error, params, reload, setPage, setFilters } =
    usePagedQuery(getRedeemCodesApi, { initial: { size: 20, sort: '-created_at' } as RedeemCodeQuery });
      ...
      <Card><CardContent className="flex flex-wrap items-end gap-3 p-4">
        <div className="flex-1 min-w-[180px]">
          ...
          <div className="relative">
            ...
            <Input className="pl-8" placeholder="搜索兑换码名称" value={params.keyword ?? ''} onChange={(e) => setFilters({ keyword: e.target.value })} />
          </div>
        </div>
...
function RedemptionsDrawer({ codeId, onClose }: { codeId: string | null; onClose: () => void }) {
  const { data, loading, error, reload } = useAsync(
    () => (codeId ? getCodeRedemptionsApi(codeId, { size: 20, sort: '-created_at' }) : Promise.resolve(null)),
    [codeId],
  );
  return (
