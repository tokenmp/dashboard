...
import { getUsersApi, getUserDetailApi } from '@/api/user';
...
function Users() {
  ...
  const { list, total, page, size, loading, error, params, reload, setPage, setFilters } =
    usePagedQuery(getUsersApi, { initial: { size: 20, sort: '-created_at' } as UserListQuery });
        ...
        <CardContent className="flex flex-wrap items-end gap-3 p-4">
          <div className="flex-1 min-w-[200px]">
            ...
            <div className="relative">
              ...
              <Input className="pl-8" placeholder="搜索邮箱"
...
function DetailDrawer({ id, onClose }: { id: string | null; onClose: () => void }) {
  const { data, loading, error } = useAsync(
    () => (id ? getUserDetailApi(id) : Promise.resolve(null)),
    [id],
  );
  return (
