...
import { getRequestLogsApi, getRequestLogDetailApi } from '@/api/request-log';
...
function Requests() {
  ...
  const { list, total, page, size, loading, error, params, reload, setPage, setFilters } =
    usePagedQuery(getRequestLogsApi, {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 筛选条 */}
      <FiltersBar params={params} onFilter={setFilters} onReset={() => setFilters({ keyword: '', model: '', protocol: '', billingPlan: '', usageStatus: '', success: '' })} />
...
function DetailDrawer({ id, onClose }: { id: string | null; onClose: () => void }) {
  const { data, loading, error } = useAsync(
    () => (id ? getRequestLogDetailApi(id) : Promise.resolve(null)),
    [id],
  );
